...
/*
* License MIT 
* https://github.com/p-m-p/API-Cache
*/
...
class API_cache {
  ...
  private 
    ...
    , $_reader; // API call (URL with params)

  public function __construct ($reader, $int=10, $cf='api_cache.json') {
    $this->_reader = $reader;
  ...
  private function _update_cache () {
    ...
    if ($fp) {
      if (flock($fp, LOCK_EX)) {
        $apiData = $this->_reader->call_remote_api();
